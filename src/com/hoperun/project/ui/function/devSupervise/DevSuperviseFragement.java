 ...
 */
package com.hoperun.project.ui.function.devSupervise;
...
import com.hoperun.manager.adpter.devSupervise.DevSuperviceListAdapter;
 ...
 */
public class DevSuperviseFragement extends PMIPBaseSecondFragment implements OnItemClickListener, OnClickListener
{
    /** 设备运行 一级栏目adpter **/
    private DevSuperviceListAdapter       fileListAdapter;
    ...
    public static DevSuperviseFragement newInstance(String parentspath, String funcId, String funName)
    {
        DevSuperviseFragement details = new DevSuperviseFragement();
                ...
                case RequestTypeConstants.GETDOCMODULELIST:
                    ...
                    if (responseBuzBody != null && responseBuzBody.getRetError().equals("0"))
                    {
                        ...
                        if (!ret.get(0).get("webitems").equals(""))
                        {
                            ...
                            GlobalState.getInstance().setmDevSuperviceModule(resultList);
                            fileListAdapter = new DevSuperviceListAdapter(mActivity, resultList);
    ...
    private void getDocModuleList()
    {
        ...
        if (GlobalState.getInstance().getmDevSuperviceModule() != null
            && GlobalState.getInstance().getmDevSuperviceModule().size() > 0)
        {
            List<HashMap<String, Object>> wetitem = GlobalState.getInstance().getmDevSuperviceModule();
            
            fileListAdapter = new DevSuperviceListAdapter(mActivity, wetitem);
        ...
        if (offLine)
        {
            ...
            {
                GlobalState.getInstance().setmDevSuperviceModule(queryret);
                fileListAdapter = new DevSuperviceListAdapter(mActivity, queryret);
                mLvMain.setAdapter(fileListAdapter);
            }
            else
            {
