...
import com.hoperun.manager.adpter.nettv.NettvListAdapter;
 ...
 */
public class NettvFragment extends PMIPBaseSecondFragment implements OnItemClickListener, OnClickListener
{
    /**
     * 网络电视
     */
    ...
    /** 网络电视一级栏目adpter **/
    private NettvListAdapter        nettvListAdapter;
    ...
    // 查询回来的list数据
    private List<String>            nettvDatas;
     ...
     * 获取获取网络电视一级列表的task实例
     */
    private NetTask                 mGetNettvModuleHttpCreator = null;
            ...
            switch (requestType)
            {
                case RequestTypeConstants.GETNETTVLIST:
                    ...
                    if (obj != null)
                    {
                        ...
                        nettvListAdapter = new NettvListAdapter(mActivity, responseBuzBody);
     ...
     */
    public static NettvFragment newInstance(String funcId, String funName, String parentsPath)
    {
        NettvFragment details = new NettvFragment();
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3)
    {
        ...
        HashMap<String, String> item = (HashMap<String, String>)nettvListAdapter.getItem(arg2 - 1);
        ...
        nettvListAdapter.setmSelectedPosition(arg2 - 1);
    ...
    private class InsertList implements Runnable
    {
        ...
        public void run()
        {
            GetNextModuleListInfo test1 =
                new GetNextModuleListInfo(MessageIdConstants.GETNETTVMODULELIST, user, funName);
    ...
    {
        NetRequestController.stopCurrentNetTask(mGetNettvModuleHttpCreator);
    }
    ...
    public boolean onKeyDown(int keyId)
    {
        ...
        if (keyId == KeyEvent.KEYCODE_BACK && isCanClose)
        {
            if (isLoading)
            {
                closeNetRequest(RequestTypeConstants.GETNETTVLIST);
                closeProgressDialog();
            }
            closeThisFragment();
            return true;
    ...
    {
        if (GlobalState.getInstance().getmNettvFirstModule() != null
            && GlobalState.getInstance().getmNettvFirstModule().getBuzList() != null
            && GlobalState.getInstance().getmNettvFirstModule().getBuzList().size() != 0)
        {
            MetaResponseBody responseBuzBody = GlobalState.getInstance().getmNettvFirstModule();
        ...
        if (offLine)
        {
            ...
            if (queryret != null && queryret.size() != 0)
            {
                ...
                GlobalState.getInstance().setmNettvFirstModule(m);
                nettvListAdapter = new NettvListAdapter(mActivity, m);
                mLvMain.setAdapter(nettvListAdapter);
            }
            else
            {
        ...
        {
            mGetNettvModuleHttpCreator = new HttpNetFactoryCreator(RequestTypeConstants.GETNETTVLIST).create();
            JSONObject body = new JSONObject();
            try
            {
                body.put("funcid", funcId);
                body.put("type", ConstState.MENU);
                NetRequestController.getNextModuleList(mGetNettvModuleHttpCreator,
                    mHandler,
