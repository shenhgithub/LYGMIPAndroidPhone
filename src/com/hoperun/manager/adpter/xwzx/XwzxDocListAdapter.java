 ...
 */
public class XwzxDocListAdapter extends PMIPBaseAdapter
{
    /** 上下文 **/
    ...
    List<HashMap<String, Object>> buzList           = new ArrayList<HashMap<String, Object>>();
     ...
     * @param list 列表数据
     */
    public XwzxDocListAdapter(Context context, MetaResponseBody listMeta)
    {
        super();
        this.context = context;
        this.list = listMeta;
        if (null != list.getBuzList())
        {
            for (int i = 0; i < list.getBuzList().size(); i++)
            {
                if (((String)list.getBuzList().get(i).get("funccode")).equals(ConstState.XWZX_TZGG))
                {
                    continue;
                }
                buzList.add(list.getBuzList().get(i));
            }
        }
    ...
    public String getItemName(int position)
    {
        ...
        Object funName = buzList.get(position).get("funcname");
        if (funName == null)
        {
    ...
    public View getView(int position, View convertView, ViewGroup parent)
    {
        ...
        viewHold.mTvName.setText((String)buzList.get(position).get("funcname"));
        if (ConstState.XWZX_SPXW.equals((String)buzList.get(position).get("funccode")))
        {
            viewHold.mImagIcon.setImageResource(R.drawable.icon_tv);
        }
